...
# @livekit/rtc-node's native (Rust) engine does its own HTTPS calls —
# e.g. LiveKit Cloud's region lookup during room connect — using the
# system TLS trust store, which a slim image doesn't include by default.
# Without it, those calls fail with an opaque "engine: signal failure".
RUN apt-get update \
